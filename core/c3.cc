...
void C3::AddAugmentedCost(const vector<MatrixXd>& G, const vector<VectorXd>& WD,
  ...
  // If this is the final solve, we may want to scale the augmented cost
  if (is_final_solve) {
    auto Gfinal = G;
    for (int i = 0; i < N_; ++i) {
      Gfinal.at(i) *= options_.final_augmented_cost_scaling.value_or(1.0);
      DRAKE_ASSERT(Gfinal.at(i).cols() == Gfinal.at(i).rows());
      augmented_costs_.push_back(prog_.AddQuadraticCost(
          2 * Gfinal.at(i), -2 * Gfinal.at(i) * delta.at(i), z_.at(i), 1));
    }
  } else {
