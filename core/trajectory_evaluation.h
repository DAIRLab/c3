...
double ComputeQuadraticTrajectoryCost(const std::vector<Eigen::VectorXd>& x,
                                      const std::vector<Eigen::VectorXd>& u,
                                      const std::vector<Eigen::VectorXd>& x_des,
                                      const std::vector<Eigen::VectorXd>& u_des,
                                      const std::vector<Eigen::MatrixXd>& Q,
...
/// Special case: cost does not depend on the contact forces, and cost matrices
/// are the same across all time steps.
double ComputeQuadraticTrajectoryCost(const std::vector<Eigen::VectorXd>& x,
                                      const std::vector<Eigen::VectorXd>& u,
                                      const std::vector<Eigen::VectorXd>& x_des,
                                      const std::vector<Eigen::VectorXd>& u_des,
...
/// Special case: cost does not depend on the inputs or contact forces, and cost
/// matrices are the same across all time steps.
double ComputeQuadraticTrajectoryCost(const std::vector<Eigen::VectorXd>& x,
                                      const std::vector<Eigen::VectorXd>& x_des,
                                      const Eigen::MatrixXd& Q);

/// From a planned trajectory of states and inputs and sets of Kp and Kd gains,
/// use an LCS to simulate tracking the plan with PD control (with feedforward
/// control), and return the resulting trajectory of actual states and inputs.
...
std::vector<Eigen::VectorXd> SimulateLCSOverTrajectory(
    const Eigen::VectorXd& x_init, const std::vector<Eigen::VectorXd>& u_plan,
    const LCS& lcs);
/// Special case: a full trajectory of states is provided, but only the initial
/// state is used for simulation.
std::vector<Eigen::VectorXd> SimulateLCSOverTrajectory(
...
std::vector<Eigen::VectorXd> SimulateLCSOverTrajectory(
    ...
    const LCS& coarse_lcs, const LCS& fine_lcs);
/// Special case: simulate a trajectory from a coarser LCS with a finer LCS,
/// where a full trajectory of states is provided but only the initial state is
/// used.
std::vector<Eigen::VectorXd> SimulateLCSOverTrajectory(
    const std::vector<Eigen::VectorXd>& x_plan,
...
std::vector<Eigen::VectorXd> ZeroOrderHoldTrajectory(
    const std::vector<Eigen::VectorXd>& coarse_traj, const int& timestep_split);
...
DownsampleTrajectories(const std::vector<Eigen::VectorXd>& x_fine,
                       ...
                       const int& timestep_split);
/// Same as above but only one trajectory is provided and processed.
std::vector<Eigen::VectorXd> DownsampleTrajectory(
...
/// horizon is an integer multiple of the coarser horizon, and that dt*N is the
/// same for both LCSs. Returns the integer multiple by which the finer horizon
/// is larger than the coarser horizon (i.e. the number of finer time steps per
/// coarser time step).
/// NOTE: This function does not assert that the LCSs' lambda dimensions are the
/// same, since it may be desirable to use different contact pairs for each LCS.
/// The state and input dimensions must be the same.
int CheckCoarseAndFineLCSCompatibility(const LCS& coarse_lcs,
                                       const LCS& fine_lcs);

/// Check that a trajectory of states, inputs, and contact forces is compatible
/// with an LCS, i.e. that the trajectory has the correct number of time steps
/// and that the dimensions of the states, inputs, and contact forces match the
/// dimensions of the LCS.
