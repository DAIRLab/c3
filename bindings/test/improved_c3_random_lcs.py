...
def make_gen_lcs_costs(lcs: LCS) -> ImprovedC3CostMatrices:
    N = lcs.N()
    n = lcs.num_states()
    m = lcs.num_lambdas()
    k = lcs.num_inputs()

    R = [0.001 * np.eye(k) for _ in range(N)]
    Q = [8500 * np.eye(n) for _ in range(N + 1)]

    Ginit = np.zeros((n + 2 * m + k, n + 2 * m + k))
    Ginit[n + m + k : n + 2 * m + k, n + m + k : n + 2 * m + k] = 2 * np.eye(m)
...
##### Systems that new C3 can stabilize #####

# case 1: n_x = 6, n_u = 2, n_lambda = 15, stiffness = 4.0, N = 5, admm_iter = 5, rho_scale = 3.3
# case 2: n_x = 6, n_u = 2, n_lambda = 15, stiffness = 4.0, N = 5, admm_iter = 5, rho_scale = 3.3
...
def main():
    ...
    options = C3Options()
    options.admm_iter = 5
    options.rho_scale = 3.3
    options.num_threads = 5
    options.delta_option = 0

    opt = ImprovedC3(gen_lcs, costs, xd, options)

    x0 = np.linspace(-5, 5, n_x)
    ...
    ax.set_title(
        r"Improved C3 Controller on a synthetic LCS with $n_x = 6, n_u = 2, n_{\lambda}=91$"
    )
    ax.grid(True)
    plt.savefig("/home/hienbui/git/c3/improved_c3_random_lcs.png", dpi=600)
